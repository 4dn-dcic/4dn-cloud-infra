...
from .utils import ( confirm_with_user, exit_with_no_action, expand_json_template_file, generate_s3_encrypt_key, print_directory_tree )
from .defs import ( ConfigTemplateVars, Directories, Files, SecretsTemplateVars )
...
def get_fallback_identity(env_name: str):
    ...
    TODO: Confused because though the 'identity' name is generated by
          datastore.application_configuration_secret it seems to be required
          in in config.json; get exception on ConfigManager.get_config_setting(Settings.IDENTITY)
          in datastore.py:application_configuration_secret if it is not set.
          I'd set it manually from the beginning (had originally gotten this verbatim as
...
#   identity_value = c4name_datastore.logical_id(camelized_env_name + identity_suffix)
    ...
    try:
        #
        # All of a sudden getting (2022-06-06 early morning):
        # Too many matches for <built-in method match of re.Pattern object at 0x1030711c0>:
        # ['vpc-es-fourfront-hotseat-pm4t3zyzz3nl7a3topmo4tqb2i.us-east-1.es.amazonaws.com',
        #  'vpc-es-fourfront-webdev-lhe3i52mydgr7sgcurmejll5ye.us-east-1.es.amazonaws.com',
        #  'vpc-es-fourfront-mastertest-b3j36tdu7mwrgdxt6y7jt46dem.us-east-1.es.amazonaws.com']
        #
        # Not getting above anymore (2022-06-06 late morning):
        # But noticed I need to have AWS_DEFAULT_REGION set or get error (from somewhere):
        # You must specify a region.
        #
        # And this is not really working in any case because though the 'identity' name is
        # generated by datastore.application_configuration_secret it seems to be required
        # in in config.json; get exception on ConfigManager.get_config_setting(Settings.IDENTITY)
        # in datastore.py:application_configuration_secret if it is not set.
        #
        # Really only need CName to call this from datastore ...
        # self.name.logical_id(camelize(ConfigManager.get_config_setting(env_name)) + self.APPLICATION_CONFIGURATION_SECRET_NAME_SUFFIX)
        # CName(name='c4-datastore-cgap-supertest', raw_name=None, title_token='C4DatastoreCgapSupertest'
        # No nevermind ... where did title_token come from ... we don't want
        # Maybe need just a new CName method to get this ... Idunno ...
        #
        # STILL TODO! ...
        #
        #importing this wants custom/config.json to exist
        #from src.stacks.alpha_stacks import c4_alpha_stack_name
        #x = c4_alpha_stack_name(name='datastore')
        #print('ffffffffffffff')
        #print(x)
        #from src.stacks.alpha_stacks import c4_alpha_stack_name
        #x = c4_alpha_stack_name('datastore')
        print('xyz')
        #print(x)
        #identity_value = c4_datastore_stack.parts[0].application_configuration_secret_default(env_name).Name
        ...
        #identity_value = C4Datastore.application_configuration_secret_default(x, "")
        #identity_value = C4Datastore.application_configuration_secret_default(None, "")

        from ...names import get_global_application_configuration_secret_name
        identity_value = get_global_application_configuration_secret_name(env_name)
