...
def _call_main(pre_existing_s3_encrypt_key_file: bool = True):
    ...
    with _setup_filesystem(Inputs.env_name, Inputs.account_number) as (aws_dir, env_dir, custom_dir), \
        ...
        mock_os_getlogin.return_value = Inputs.deploying_iam_user
...
def _test_main_exit_with_no_action_on_missing_required_input(omit_required_arg: str):
    ...
    if omit_required_arg == "--account":
        ...
        account_number = None
    with _setup_filesystem(Inputs.env_name, account_number) as (aws_dir, env_dir, custom_dir), \
         mock.patch("src.auto.init_custom_dir.cli.PRINT"), \
         mock.patch("src.auto.init_custom_dir.utils.PRINT"), \
         mock.patch("builtins.input") as mock_input:
        argv = _get_standard_main_argv(aws_dir, Inputs.env_name, custom_dir, omit_arg=omit_required_arg)
