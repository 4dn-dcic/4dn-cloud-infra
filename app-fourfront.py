...
def get_view_reload_lambda_route(lambda_name):
    ...
    environ = os.environ.get("ENV_NAME")
    is_admin = app_utils_manager.singleton.check_authorization(req_dict, environ)
    return app_utils_manager.singleton.view_reload_lambda(request=app.current_request, is_admin=is_admin, domain=domain, context=context, lambda_name=lambda_name)
