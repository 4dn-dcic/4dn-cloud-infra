...
SHARED_KEYS = {
    ...
    "iam.ecosystem_resources": "iam_ecosystem_resources",
    "network.flow_logs.enabled": "flow_logs_enabled",
    "network.flow_logs.retention_days": "flow_logs_retention_days",
}
ENV_KEYS = {
    ...
    "rds.name": "rds_name",
    "rds.postgres_version": "rds_postgres_version",
    "rds.storage_type": "rds_storage_type",
    "rds.backup_retention_days": "rds_backup_retention",
    "ecs.lb_certificate_arn": "certificate_arn",
    "codebuild.log_retention_days": "log_retention_days",
...
def _emit(title, mapping, config):
    ...
    for cfg_key, tf_var in mapping.items():
        if cfg_key in config:
            v = config[cfg_key]
            if cfg_key == "app.deploy":
                v = {"blue/green": "blue_green"}.get(v, v)
            # JSON strings are valid HCL strings; escaping prevents quotes/newlines becoming code.
