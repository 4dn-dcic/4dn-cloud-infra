...
def lookup_stack_creator(name, kind, exact=False):
    ...
    for stack_name, stack_creator in REGISTERED_STACKS[kind].items():
        if exact and name == stack_name:
            return stack_creator
        elif not exact and stack_name in name:
            return stack_creator
    raise ValueError("A %s stack %s %r was not found." % (kind, "called" if exact else "whose name contains", name))
